...
public class AIAgentController : MonoBehaviour {
    ...
    float CalculateSpeed(List<float> considerationList)
    {
        return 0.0f;
    }
    ...
    void MoveTowardsDestination()
    {
        ...
        //Debug.Log("Look At to Destination Dot: " + lookAtToDestinationDot);
        //Debug.Log("Right to Destination Dot: " + rightToDestinationDot);

        float toDestinationAngle = Mathf.Rad2Deg * Mathf.Acos(lookAtToDestinationDot);
        //Debug.Log("LookAt Angle To Destination: " + toDestinationAngle);
        List<float> considerations = new List<float>();
        float deltaDistance = m_maxSpeedDistance - m_destinationBuffer;
        float distance = Mathf.Clamp(distanceToDestination, m_destinationBuffer, m_maxSpeedDistance);
        float distanceConsideration = (distance - m_destinationBuffer) / deltaDistance;

        float speed = distanceConsideration * m_agent.m_linearMaxSpeed;//CalculateSpeed(considerations);
        ...
        float deltaAngle = m_maxAngularSpeedAngle - m_minAngularSpeedAngle;
        float angle = Mathf.Clamp(toDestinationAngle, m_minAngularSpeedAngle, m_maxAngularSpeedAngle);
        float angleConsideration = (angle - m_minAngularSpeedAngle) / deltaAngle;
